...
H~A~: **pH predicts abalone growth **

## b. Cleaning

```{r abalone-data-cleaning}
# creating ablone clean from original data set 
...
# creatinf a base layer ggplot call
ggplot(data = abalone_clean, # 
       aes(x = mean_ph, # 
           y = change_in_area)) 
  # data set should be clean x axis should be predictor y axis should be response # first layer point to represent average change in shell surface for each pair of tanks
  geom_point(size = 4, # 
             stroke = 1, # get rid of missing value 
             fill = "firebrick4",
             shape = 21)
```
...
**tank B1B when the pH sensor failed**  
...
Residuals seen homoclastic (constant across values) base of residuals vs fitted and scale- location plots. residuals also seem normally distributed 


What is the slope estimate (with standard error?)  We evaluated model assumptions visually. For hoomesticity of residuals we used rediduals vs fitted and scale loaction plots. For normality, qqPlots. Based of the diagnositcs 

**2.98 se = 
= +/- .76*with eash pH the model predicts and increase of  2.98 mm ^2^ in shell surface area per day *  

What is the intercept estimate (with standard error?)  

**The intercept is -18.5 mm^2^ +/_ 6.16 when pH = 0 The model predicts that abalone shell surface changes by 18.5 +/_ 6.16. This isnt biologically realistic it is just something that the mode predicts based on the model as a estimate**
...
**x ; predictor value for PH 
predicted = model prediction for change in shell surface per day 
std error ; se precision of the prediction 
conf. low/high range for the 95% confidence interval **  
...
**at pH  = 8 the model predicts that abalone change in shell surfece area per day should be 5.36 (c=CI ; [5.98, 5.64]**
